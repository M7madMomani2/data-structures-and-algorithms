...
const getNames = (arr) => {
  // Solution code here...
  return arr.map((value) => value.name.split('').reverse().join('')); // i tried to use reduce but it didnt woked.
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let result = newArray.map(value => value.map(element => Math.pow(2,element)));
  return result;
};
...
let findShortest = (data) => {
  // Solution code here...
  let names = data.sort((elemnt1,elemnt2) => elemnt1.height - elemnt2.height );
  return names[0].name;
};

...
const speaker = (words, callback) => {
  // Solution code here...
  let arr =[];
  words.forEach(element =>arr.push((callback(element))));
  return arr;
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(element => {
    if (element.available === true){
      myShoppingList.push(element.name);
    }
  });
...
const fizzbuzz = (arr) => {
  ...
  let newArr=[];
  arr.forEach(elem => {
    if (elem %3===0&& elem %5===0){newArr.push("Fizz Buzz");}
    else if (elem %3===0){newArr.push("Fizz");}
    else if (elem %5===0){newArr.push("Buzz");}
